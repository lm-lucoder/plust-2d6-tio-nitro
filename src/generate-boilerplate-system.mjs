...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'nitro2d6', 'Nitro2d6', and 'BOILERPLATE'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.nitro2d6/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.nitro2d6/g),
   ...
   * Rename files.
   *
   * Rename files that had nitro2d6 in their name, such as
   * css/nitro2d6.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*nitro2d6*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('nitro2d6', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-nitro2d6-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Nitro2d6's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace nitro2d6 name mentions in files.
      generator.replaceFileContents();
      // Rename files that had nitro2d6 in their name.
